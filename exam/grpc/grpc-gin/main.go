...
import (
	...
	"github.com/HsiaoCz/geek/exam/grpc/grpc-gin/pb"
...
var (
	net_work  = "tcp"
	gin_addr  = "127.0.0.1:3001"
	grpc_addr = "127.0.0.1:3002"
)
...
func HandleHello(c *gin.Context) {
	hello := new(Hello)
	if err := c.BindJSON(hello); err != nil {
		log.Fatal(err)
	}
...
func main() {
	listen, err := net.Listen(net_work, grpc_addr)
	if err != nil {
		log.Fatal(err)
	}
	s := grpc.NewServer()
